...
def _collect_runtime_defs_from_bundle(
    bundle_path: Path,
    variables: dict[str, Any],
    slot_defs: dict[tuple[int, int], dict[str, Any]],
    scenario_defs: list[dict[str, Any]],
) -> None:
    """Populate slot/scenario defs by parsing a decompiled bundle.js."""
...
def _collect_runtime_defs_from_objects(
    objects: list[dict[str, Any]],
    slot_defs: dict[tuple[int, int], dict[str, Any]],
    scenario_defs: list[dict[str, Any]],
) -> None:
    """Populate slot/scenario defs from APK-reconstructed object literals."""
    for value in objects:
        if not isinstance(value, dict):
            continue
        if {"componentId", "id", "name", "datatype"} <= set(value):
            component_id = value.get("componentId")
            sensor_id = value.get("id")
            if isinstance(component_id, int) and isinstance(sensor_id, int):
                slot_defs[(component_id, sensor_id)] = value
...
def _extract_bundle_runtime_data(
    ...
    """Extract slot metadata and scenarios from bundle.js or reconstructed objects."""
    variables: dict[str, Any] = {}
    slot_defs: dict[tuple[int, int], dict[str, Any]] = {}
    scenario_defs: list[dict[str, Any]] = []

    if objects is not None:
        _collect_runtime_defs_from_objects(objects, slot_defs, scenario_defs)
    elif bundle_path.exists():
        _collect_runtime_defs_from_bundle(
            bundle_path, variables, slot_defs, scenario_defs
        )
    else:
        return {}, []
...
def _collect_registry_defs_from_bundle(
    bundle_path: Path,
) -> tuple[dict[int, dict[str, Any]], list[dict[str, Any]], dict[Any, Any]]:
    """Collect component/vehicle defs by parsing a decompiled bundle.js."""
    variables: dict[str, Any] = {}
...
def _collect_registry_defs_from_objects(
    ...
    """Collect component/vehicle defs from APK-reconstructed object literals."""
    component_defs: dict[int, dict[str, Any]] = {}
    ...
    for value in objects:
        ...
        if isinstance(group_variant, dict) and len(group_variant) >= len(
            ...
            vehicle_group_variant = group_variant
    return component_defs, list(seen_vehicles.values()), vehicle_group_variant
...
def _extract_bundle_registry_data(
    ...
    """Extract component/vehicle metadata from bundle.js or reconstructed objects."""
    if objects is not None:
        component_defs, vehicles, vehicle_group_variant = (
            _collect_registry_defs_from_objects(objects)
        )
    elif bundle_path.exists():
        component_defs, vehicles, vehicle_group_variant = (
            _collect_registry_defs_from_bundle(bundle_path)
        )
    else:
        return {}, [], {}
    ...
    bundle_slots, _bundle_scenarios = _extract_bundle_runtime_data(bundle_path, objects)
...
def generate_overlay_from_bundle(
    ...
    # ``objects`` (APK-reconstructed literals) takes precedence over bundle.js.
    components, vehicles, vehicle_group_variant = _extract_bundle_registry_data(
        bundle_path, objects
    )
    component_display_names = _extract_component_display_names_from_bundle(
        bundle_path, objects
    )
    controls = _control_profiles()
    slot_controls = _slot_control_profiles()
    bundle_slots, bundle_scenarios = _extract_bundle_runtime_data(bundle_path, objects)
    legacy_transform_hints = _extract_legacy_transform_hints(pia_decoder_path)
    canonical_provider_slots = _canonical_provider_slots(provider_specs_path)
